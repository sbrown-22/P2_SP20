...
# by tagname
title = soup.find("title")  # look for the title tag <title> and returns the first
...
print(big_font)
print(big_font[0].text)  # iterable, can index, has attributes
...
print(quote_divs[0].span)  # or use dot notation to drill down

quote_list = []
for quote in quote_divs:
    quote_list.append(quote.span.text)

quote_list = [x.span.text for x in quote_divs]  # same as above
...
print(first_author_tag['itemprop'])  # itemprop is equal to "author"
